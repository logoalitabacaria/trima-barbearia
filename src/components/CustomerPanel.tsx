...
export default function CustomerPanel({
  ...
  // Service Description Expansion State (Down Arrow toggle)
  ...
  return (
    <div className="space-y-6 text-left max-w-6xl mx-auto font-sans bg-slate-50 p-4 sm:p-6 rounded-3xl shadow-sm border border-slate-200/80">
      
      {/* GUEST BANNER NOTIFICATION */}
      {isGuestMode && (
        <div className="bg-amber-500 text-slate-950 p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 font-mono shadow-md border border-amber-400">
          ...
          <button
            ...
            className="px-4 py-2 bg-slate-950 hover:bg-slate-900 text-amber-400 text-xs font-black uppercase rounded-xl transition shrink-0 cursor-pointer flex items-center gap-1.5 shadow"
          >
      ...
      {/* MOBILE QUICK SHORTCUTS BAR */}
      <div className="md:hidden grid grid-cols-4 gap-2 bg-white border border-slate-200 p-2 rounded-2xl shadow-xs">
        <button
          type="button"
          onClick={() => setActiveTab('agendar')}
          className={`p-2 rounded-xl text-[10px] font-bold font-mono flex flex-col items-center gap-1 transition ${
            activeTab === 'agendar' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
        ...
        <button
          ...
          className={`p-2 rounded-xl text-[10px] font-bold font-mono flex flex-col items-center gap-1 transition ${
            activeTab === 'assinatura' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
        ...
        <button
          ...
          className={`p-2 rounded-xl text-[10px] font-bold font-mono flex flex-col items-center gap-1 transition ${
            activeTab === 'historico' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
        ...
        <a
          ...
          className="p-2 rounded-xl text-[10px] font-bold font-mono text-slate-600 hover:bg-slate-100 flex flex-col items-center gap-1"
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-left select-none flex-1">
                              ...
                              <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                {s.description && (
                                  <button
                                    ...
                                    title={expandedServiceIds[s.id] ? 'Ocultar descrição' : 'Ver descrição completa'}
                                  >
                                    <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${expandedServiceIds[s.id] ? 'rotate-180 text-amber-600' : ''}`} />
                                  </button>
                                )}
                              </div>
                            ...
                            {/* Service Description - Collapsible or Preview */}
                            {s.description && (
                              expandedServiceIds[s.id] ? (
            ...
            <div className="bg-white border border-slate-200/90 p-5 rounded-2xl space-y-4 shadow-sm flex flex-col justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-500 mb-3">Selecione quem fará seu atendimento:</p>
                  ...
                  {barbers.map(b => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        {b.photoUrl ? (
                          <img src={b.photoUrl} alt={b.name} className="h-12 w-12 object-cover rounded-xl border border-slate-200" referrerPolicy="no-referrer" />
                        ) : (
                          <div className="w-12 h-12 bg-amber-100 border border-amber-200 text-slate-900 rounded-xl flex items-center justify-center text-2xl font-bold">
                            {b.avatar || '🧔'}
                          </div>
                        )}
                        <div className="text-left select-none flex-1">
                          <h4 className="font-extrabold text-xs text-slate-900">{b.name}</h4>
                          <span className="text-[9px] uppercase font-mono font-bold text-amber-700 bg-amber-100/80 px-2 py-0.5 rounded inline-block mt-0.5">
                            Barbeiro Profissional
                          </span>
                        </div>
                        {bookingBarberId === b.id && (
                          <Check className="w-5 h-5 text-amber-600 font-bold" />
                        )}
                      </div>
                      <p className="text-[10px] text-slate-600 mt-2.5 border-t border-slate-200/60 pt-2 leading-relaxed">
                        {b.bio || 'Especialista em cortes modernos, acabamento clássico e visagismo.'}
                      </p>
                    </label>
                  ))}
                </div>
            ...
            <div className="bg-white border border-slate-200/90 p-5 rounded-2xl space-y-4 shadow-sm flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-3">
                  <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl space-y-2">
                    <div className="flex justify-between items-center text-xs font-bold uppercase font-mono">
                      <button
                        ...
                        className="p-1 px-2.5 bg-white border border-slate-200 rounded hover:bg-slate-100 text-slate-700 transition cursor-pointer"
                      >
                        &larr;
                      </button>
                  ...
                  {/* Time Slots */}
                  <div className="pt-2">
                    <label className="text-[10px] text-slate-600 font-mono block uppercase font-bold mb-1.5">
                      Horários para {bookingDate.split('-').reverse().join('/')}:
                    </label>
                    ...
                    {(() => {
                      ...
                      if (slots.length === 0) {
                        return (
                          <p className="text-[11px] text-slate-400 italic font-mono bg-slate-100 p-2 text-center rounded-lg">
                          ...
                          {slots.map(slot => {
                            ...
                            return (
                              <button
                                ...
                                className={`py-1.5 text-[10px] font-mono rounded-lg border transition-all text-center font-bold ${
                                  isBooked
                                    ? 'bg-slate-100 border-slate-200 text-slate-400 line-through cursor-not-allowed'
                                    : isSelected
                                      ? 'bg-amber-500 border-amber-600 text-slate-950 shadow-md scale-105'
                                      : 'bg-slate-50 border-slate-200 hover:border-amber-400 hover:bg-amber-50 text-slate-700 cursor-pointer'
                                }`}
                              >
                                {slot}
                              </button>
